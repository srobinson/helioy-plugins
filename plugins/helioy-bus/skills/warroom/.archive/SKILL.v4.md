...
5. Prime reviewers with skills `/code-review` (correctness bugs, reuse, simplification) and `/code-hygiene` (decomposition, duplication, boundaries) before they read any diff, so the pass runs on those disciplines rather than ad hoc judgment. **Default: pre-load via `tmux send-keys`** (runtime prefix applies: `/` on Claude, `$` on Codex), NOT brief-time invocation. Pre-loading is verifiable and decoupled: you confirm via `capture-pane` that the disciplines actually loaded before any brief lands, and the review brief then stays about the artifact. **Brief-time invocation** (telling the reviewer in the bus brief to invoke the skills first) is a **fallback only when send-keys is unavailable** (MCP-only, no tmux access): it is unverifiable, races the review, and the agent can silently skip it and review ad hoc. To pre-load: type the skill commands into the pane, verify each submitted via `capture-pane`, then immediately follow with a one-line "parking for later" note (the skills are priming for an upcoming brief; do not review anything now) so the invocation does not trigger an ad hoc pass on whatever sits in the working tree. The parking note MUST carry its own expiry: "when the review brief arrives over the bus, proceed immediately without asking for further confirmation." Without it, a cautious reviewer treats the brief as ambiguous against the standing hold and stalls on a should-I-proceed menu nobody is watching. Verify EVERY send-keys line submitted, the parking note included: any line can be left sitting unsubmitted in the input box, and a busy pane eats Enters; if the capture shows text still at the prompt, send a bare `Enter` and re-check. When you pre-load, the later review brief says "proceed (already primed)", NOT "invoke /code-review first". State that the no-writes rule extends to any subagents the reviewer spawns: a finder subagent will happily edit the shared checkout mid-review unless told otherwise, and the reviewer must verify the tree is pristine before delivering a verdict.
...
3. Brief the reviewer for one adversarial pass against the PR. Prime the reviewer with `/code-review` and `/code-hygiene` first, per Mode 3 step 5 — **pre-load via send-keys by default** (verify each submitted via `capture-pane`), with brief-time invocation only as a fallback when send-keys is unavailable. Findings are Blocker, Major, or Minor with `file:line`.
