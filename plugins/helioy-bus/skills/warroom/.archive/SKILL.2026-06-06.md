...
### Skip the warroom when it adds no catch rate

The cheapest MoE panel is the one you never spawn. Do the work directly in your own session, no warroom, when **all** of these hold:

- The change is mechanically locked: a one-to-three-line edit with a single obvious implementation (a rename, a lint attribute, a deletion, toml inheritance).
- The design is already adjudicated upstream (a prior review, a design doc, or an earlier item in this batch locked the mechanics).
- Your own verification gate (the same `cargo`/`just`/test commands the warroom would run) is sufficient evidence the change is correct.

Spawn a pair only when the engineer's design choices are genuinely open and a misclassification would otherwise ship. The verification gate is the load-bearing safety; the warroom cycle is not.
...
Each brief tells the agent: *"Talk directly to your peer. Address every message `to=\"<peer>;<me>\"` so I'm CC'd, keep it to one typed line citing IDs, paths, and SHAs, and route the debate to your peer. I will not relay."*
...
8. **Message discipline** — *"One typed line per message. Cite Linear IDs, paths, PR numbers, and SHAs; never paste the artifact, your peer and I read it live via MCP. Send `to=\"<peer>;<orchestrator>\"`, `reply_to` your peer."*
...
Bounded flow. Each line is one typed message `to="<peer>;<orchestrator>"`:

```
A → one-line critique citing evidence, OR sign-off
B → one-line critique citing evidence, OR sign-off
[if either blocked] author addresses it, re-posts one changed line   # one correction round
A → "I sign off conditional on the following changes: …"
B → "I sign off conditional on the following changes: …"
```

Debate budget: one critique plus one response per round, then one correction, then sign-off or escalate with a single `E` line to the orchestrator. No open-ended prose on the bus.
...
- **Token economy (read this first).** The bus carries typed signals, not conversation. Every message is one line. Cite paths, symbols, SHAs, Linear IDs, and PR numbers. Never paste docs, diffs, test logs, or review essays; peers fetch and read directly via MCP, `git diff`, or `Read`. Each round is one proposal plus one review response. If blocked, one correction, then sign-off or escalate. No open-ended back-and-forth on the bus. The `moe-local-batch` workflow (`helioy-tools:workflows`) is the fully worked, token-disciplined instantiation of this for a batch of N small code changes.
...
- **CC via `;` recipients.** helioy-bus has no separate cc field, but `send_message(to=…)` accepts a `;`-joined list, so `to="<peer>;<orchestrator>"` delivers one message to both. That is the CC mechanism for the peer-to-peer modes: agents address each direct message to their peer and you, so you observe without relaying.
- **CC only typed lines, never prose.** A CC'd copy must be a single typed line: a sign-off, a block, or a milestone. Full critique and evidence stay peer-to-peer or in files. CC-ing full debate prose dumps the whole exchange into your context, the most expensive in the system, and is the chattiness to avoid.
- **Reply-to pattern.**
  - **Peer-consensus.** Send `to="<peer>;<orchestrator>"`; `reply_to` defaults to the sender so the peer replies straight back and the debate stays between the panes.
  - **Spec-writing.** `reply_to` is the counterpart (engineer ↔ architect); CC the orchestrator via `;`.
...
| Paste full prose, diffs, or logs into the bus | One typed line citing IDs, paths, and SHAs; peers read the artifact directly. CC the orchestrator only on typed lines. |
| Spawn a warroom for a mechanically-locked change | Do it directly in your own session; the verification gate is the safety, the warroom only adds latency. |
...
| Pass the same plugin-namespaced agent twice to `warroom_spawn` expecting MoE | Both panes land on the default adapter (claude). Spawn once, then `warroom_add(..., runtime="codex")` for the second pane. The `runtime` arg on `warroom_spawn` applies to all spawned panes, so a single call cannot mix runtimes. |
