...
Iterated mixture-of-experts peer-consensus review of a Linear issue set against the current codebase. Each pass runs the same fresh-eyes audit with new pane contexts until a round-1 review surfaces zero substantive blockers.

## Purpose

Single-pass review catches the obvious defects. MoE iterated review catches the assumptions the drafter and the first reviewer carried forward together. The main job is to test issue substance against current source, tests, generated surfaces, command contracts, and runtime behaviour. Linear shape matters, but codebase fit is the primary review surface.
...
Use the mixture-of-experts default per [Warroom Mode 1: Peer Consensus](../../../../helioy-bus/skills/warroom/SKILL.md#mode-1-peer-consensus): the same `helioy-tools:*` agent on Claude and Codex panes. Same prompt, different model on each pane. Disagreement between panes is signal about the artifact; agreement under fresh-eyes pressure is the exit condition.

Exact spawn shape — do not deviate, do not collapse into a single `warroom_spawn` call with two agents (both panes would land on the default adapter):

```
warroom_spawn(name="moe-{topic}-pass{N}", agents=["helioy-tools:codebase-analyst"], cwd="<repo under review>")
warroom_add(name="moe-{topic}-pass{N}", agent="helioy-tools:codebase-analyst", runtime="codex")
```

The `runtime="codex"` on the add is load-bearing: a plugin-namespaced agent name does not select a non-default runtime by itself.
...
The review signal comes from fresh contexts, cold artifact reads, independent first findings, peer reconciliation, orchestrator-owned edits, and live verification after edits. The bus thread should carry only the state needed to drive those actions.

Default to this protocol for every pass.

## Codebase Substance Standard

MoE issue review is a codebase-grounded design review. Panes must inspect the actual repo before signing off. They validate that each issue's requested capability, acceptance criteria, order, and verification path fit the current implementation and runtime contracts.

Required checks:

- Read the current entry points named by the issues, then follow the call path far enough to validate the proposed work.
- Check tests, generated surfaces, schemas, docs, and command definitions when they are part of the user-visible or agent-visible contract.
- Validate runtime behaviour from current code and lightweight commands where feasible. Do not implement planned changes during review. When runtime proof requires implementation or unsafe setup, file a finding that tightens the issue's acceptance or verification path.
- Suggest simpler, safer, more durable, or better-factored designs when the code indicates a better path.
- If a materially different design, scope tradeoff, or product decision is proposed, emit an `E|ALT|...` escalation. Human decision is required before the orchestrator applies that alternative.

### Compact pass card

Send each pane the same card. The card indexes the work for the panes. Panes must fetch live Linear and inspect current code before signing off.

```text
MOE_PASS v1
Artifact: <master/gate/backlog/PER issue IDs>
Topic: <artifact>-review-pass<N>
Peer: <peer agent_id>
Workflow: helioy-tools:linear-workflows / MoE Issue Review
Read: live Linear, current source, tests, generated surfaces, command/runtime contracts
Scope: codebase substance, design fit, selector shape, reviewability; no implementation
Probes: <3-6 defect-class ids from the catalog below>
Focus: <1-3 current questions, phrased as surfaces to test>
Signoff target: <artifact name>
Protocol: F, A, S, V, or E messages only
```

Do not include prior pass findings. Do not paste the defect catalog. Use defect class names or numbers from this file.

### Message grammar

Each pane sends at most one findings message, one agreement message, and one sign-off message per round unless it must escalate.

```text
F|<id>|<issue/path>|<class>|<code evidence>|<risk>|<required change>
A|accept:<ids>|reject:<id:reason>|missing:<probe ids or none>
S|I sign off on <target> as currently filed
S|I sign off conditional on the following changes: <numbered finding ids plus one-line edits>
V|I sign off on <target> as currently filed
E|<one unresolved decision or blocker>
E|ALT|<finding id>|<alternative>|<human decision needed>
```

Rules:

- `F` lines are one finding each. No quoted issue bodies, no background narrative.
- Each substantive `F` line cites code evidence: `path`, `path:symbol`, test name, generated surface, command definition, runtime contract, or `missing-proof:<specific gap>`.
- Accepted findings need no rationale. Rejections get one reason.
- `A` messages reconcile by finding id. They do not restate findings.
- `S` uses the exact sign-off phrase. Conditional sign-off references finding ids and one-line edits.
- `V` is used after orchestrator edits. The pane must re-read live state first and send only the clean sign-off or `E`.
- `E` ends the compact path. The orchestrator resolves the decision, then resumes with a fresh compact pass or stops. `E|ALT` means a materially different design or tradeoff needs human choice.

### Debate budget

Round 1 has three turns per pane:

1. Independent `F` list.
2. One `A` line responding to peer findings.
3. `S` clean or conditional.

If the panes still disagree after the `A` turn, they do not debate by essay. Each sends one `E` line naming the unresolved decision. The orchestrator decides, edits, or escalates to the user.

### Compact verify nudge

After applying consensus changes, send:

```text
VERIFY v1
Artifact: <issue ids>
Applied: <finding ids>
Required: re-read live Linear, then send V or E only
```

### Compact cm record

Persist the pass as structured fields with terse values:

```text
Pass: <N>
Artifact: <ids>
Round1: substantive=<count>; cosmetic=<count>
Classes: <class ids>
Accepted: <finding ids>
Rejected: <finding ids and one-line reason>
Outcome: continue | formal-exit | smell-exit | escalate
Curve: <counts by pass>
Lesson: <one sentence, only if reusable>
```
...
- A worker's requested design does not match current code, tests, generated output, or runtime contracts.
- The codebase reveals a simpler or safer design than the issue currently authorizes.
...
Standard peer-consensus brief from [Warroom Mode 1](../../../../helioy-bus/skills/warroom/SKILL.md#mode-1-peer-consensus), compressed through the [Token Economy Protocol](#token-economy-protocol), plus three additions:
...
- **Codebase-grounding clause.** Explicit instruction: "Review the issues against the current codebase. Read the relevant source, tests, generated surfaces, command definitions, and runtime contracts before signing off. Findings must cite code evidence or a concrete missing proof."
- **Defect-class catalog pointer.** Name the class ids to probe. Do not paste the catalog into the bus brief.
...
After each pass closes, write a compact cm decision with:
...
- **Body.** Use the [compact cm record](#compact-cm-record): accepted finding ids, defect classes touched, defect count, outcome, current cumulative defect curve, and one reusable lesson when there is one.
