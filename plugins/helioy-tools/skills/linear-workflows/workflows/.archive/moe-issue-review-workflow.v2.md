...
Send each pane the same card. The card indexes the work for the panes. Panes must fetch live Linear and referenced files before signing off.
...
Read: live Linear and referenced files only
Scope: issue quality, selector shape, reviewability; no implementation
...
```text
F|<id>|<issue/path>|<class>|<risk>|<required change>
...
- `E` ends the compact path. The orchestrator resolves the decision, then resumes with a fresh compact pass or stops.
