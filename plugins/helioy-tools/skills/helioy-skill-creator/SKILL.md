---
name: helioy-skill-creator
description: >
  Create new skills for the helioy-tools plugin. Use when the user asks to
  build, create, or add a new skill inside helioy-plugins, or when you need
  to scaffold a skill as part of a Helioy task. Enforces Helioy plugin
  conventions: correct location, frontmatter, naming, MCP inheritance, and
  Linear issue structure. For generic (non-Helioy) skill scaffolding, use
  the standard `skill-creator` instead.
---
