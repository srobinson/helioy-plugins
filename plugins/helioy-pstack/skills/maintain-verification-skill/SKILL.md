...
0. **Locate the target.** Find the verification skill to maintain: the shared project skill whose body has launch and drive sections plus a feature map, usually `.agents/skills/verify-*/`. Ask which one when several candidates exist. When none exist, stop and point at `/create-verification-skill`.
