...
Use the installed `transcript-search` skill and Transport Matters session API for transcript discovery and reading. Pin every query to the active workspace. If that capability is unavailable, use the current runtime's workspace-scoped transcript location and report the missing capability. Never scan unrelated runtime homes.
...
3. Fan out across your chat history when the corpus warrants it. Use Helioy warroom members with disjoint time ranges or topics. Each member uses `transcript-search`, skips the current run and obvious eval or test runs, and returns the same schema: topic, user goal, decisions, open threads, corrections, and artifacts, with session IDs. For one or two sessions, search directly. Raw transcripts stay with the members. The coordinator receives only findings.
