...
Read [`../poteto-mode/references/helioy-runtime.md`](../poteto-mode/references/helioy-runtime.md) before transcript fanout or skill generation. Use Transport Matters transcript search and Context Matters when available. Do not create or publish a skill without the authority granted by the current request.
...
- Keep the generated description narrowly triggerable. Mode skills are heavy and opinionated, so they should apply when the user invokes them by name or slash command. Use a runtime-specific invocation policy only when that runtime supports it. Enable broad automatic use only when the user explicitly requests it.
