...
import * as T from "./types.ts"; export { assessGitHubMerge, readSnapshot } from "./snapshot.ts";
...
function threadBlocker(row: T.PrSnapshot): T.MergeBlocker | null {
  ...
  const threads = T.nonEmpty(row.threads);
  return threads === null
    ? null
...
export async function runSimple(args: {
  ...
  const step = async (): Promise<StepResult<T.TerminalVerdict>> => {
    ...
    const complete = T.nonEmpty(rows);
