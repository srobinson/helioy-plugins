...
Two plugins bundled in one installable package:

1. **helioy-tools**: The core capability layer. Six MCP servers (geometric memory, structured context, code navigation, markdown search, Linear, Supabase), 15 skills (from session memory to PR creation), and 34 specialized agent definitions spanning engineering, design, research, and orchestration.
...
**Memory**: Two complementary systems. `attention-matters` provides geometric memory on an S3 hypersphere for associative recall. `context-matters` provides a structured SQLite store with hierarchical scopes for facts, decisions, and feedback.
